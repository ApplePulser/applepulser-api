...
- **RoomCreateView**: 방 생성
- **RoomDetailView**: 방 조회
- **JoinRoomView**: 방 참가 (응답에 player_id 포함)
- **LeaveRoomView**: 방 퇴장
...
- **플레이어 Ready 상태 브로드캐스트**: 준비 상태 변경 실시간 알림
...
```json
{
  ...
  "max_players": 4,
  "players": [
    {"player_id": "uuid-host", "nickname": "방장", "status": "waiting", "is_host": true},
    {"player_id": "uuid-xxx", "nickname": "플레이어1", "status": "waiting", "is_host": false}
  ],
...
응답: 200 OK (player_id + 방 전체 정보 + 모든 플레이어 리스트)
```
...
- **플레이어 Ready 처리**: player_ready 메시지 수신 시 DB 상태 변경 및 브로드캐스트
